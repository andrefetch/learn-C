...
### Multi-space input
We can use `fget` which means file get string it takes the variable name, the size of the variable, we can use `sizeof(var_name)` to approximate the size of the array and lastly it takes stdin as the file stream. It also produces a new line into the input buffer, results needing to use strlen(name) - 1 and assinging it to a null terminator to get rid of the "\n"
